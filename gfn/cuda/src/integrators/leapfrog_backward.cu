...
__global__ void leapfrog_backward_kernel(
    ...
    for (int step = 0; step < steps; step++) {
        christoffel_device(s_v, U, W, s_gamma1, s_x, nullptr, dim, rank, 0.0f, 1.0f, 1.0f, false, topology, nullptr, nullptr, nullptr, nullptr, s_h, s_E, s_P, s_M_f);
        __syncthreads();
        for (int i = tid; i < dim; i += blockDim.x) { s_v_half[i] = s_v[i] + 0.5f * eff_dt * ((f?f[b*dim+i]:0.0f) - s_gamma1[i]); s_x[i] += eff_dt * s_v_half[i]; }
        __syncthreads();
        christoffel_device(s_v_half, U, W, s_gamma2, s_x, nullptr, dim, rank, 0.0f, 1.0f, 1.0f, false, topology, nullptr, nullptr, nullptr, nullptr, s_h, s_E, s_P, s_M_f);
        __syncthreads();
    ...
    for (int step = steps - 1; step >= 0; step--) {
        for (int i = tid; i < dim; i += blockDim.x) { float gv = s_gv[i]; if (grad_f) atomicAdd(&grad_f[b*dim+i], gv*0.5f*eff_dt); s_gv[i] += -gv*0.5f*eff_dt; s_gx[i] += -gv*0.5f*eff_dt; s_gv[i] += s_gx[i]*eff_dt; float gvh = s_gv[i]; if (grad_f) atomicAdd(&grad_f[b*dim+i], gvh*0.5f*eff_dt); s_gv[i] += -gvh*0.5f*eff_dt; s_gx[i] += -gvh*0.5f*eff_dt; }
        __syncthreads();
    }
...
extern "C" void launch_leapfrog_backward(
    ...
    int batch, int dim, int rank, float dt, float dt_scale, int steps, int topology, cudaStream_t stream
) {
    int shared = (5 * dim + rank + 16) * sizeof(float) + (rank + 4) * sizeof(double);
    leapfrog_backward_kernel<<<batch, BLOCK_SIZE, shared, stream>>>(
        grad_x_new, grad_v_new, x, v, f, U, W, grad_x, grad_v, grad_f, grad_U, grad_W, batch, dim, rank, dt, dt_scale, steps, topology
    );
}
