...
def verify_noether_symmetries(checkpoint_path):
        ...
        for i, (s1, s2) in enumerate(pairs):
            for s, name in [(s1, f"Pair {i} A"), (s2, f"Pair {i} B")]:
                ...
                for t in range(ids.size(1)):
                    x, v = model.layers[0](x, v, forces[:, t])
    ...
    plt.title("Isomeric Manifolds: Noether Invariance Map\n(Close proximity = High Semantic Symmetry)", fontsize=14, fontweight='bold')
    plt.legend()
    
    results_dir = PROJECT_ROOT / "tests" / "benchmarks" / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(results_dir / "noether_invariance.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"✅ Noether Invariance map saved to: {results_dir}/noether_invariance.png")
