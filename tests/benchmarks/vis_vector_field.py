...
def plot_christoffel_vector_field(checkpoint_path):
    ...
    # Quiver Plot (The "Flow")
    # Note: U and V represent the 'push' exerted by the manifold at each point (v_0, v_1)
    q = plt.quiver(X, Y, U_force, V_force, magnitudes, cmap='YlGnBu', scale=50, width=0.003)
    plt.colorbar(q, label='Force Magnitude ||Γ||')
    
    plt.title("Manifold v1.0: Christoffel Vector Field\n(Geometric Forces steering the 'Thought' Velocity)", fontsize=16, fontweight='bold')
    ...
    plt.savefig(results_dir / "christoffel_vector_field.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"✅ Vector Field plot saved to: {results_dir}/christoffel_vector_field.png")
