...
def visualize_loss_landscapes():
    ...
    X_gfn, Y_gfn, Z_gfn = compute_loss_landscape(
        gfn, (inputs, targets), criterion, gfn_dir1, gfn_dir2,
        alpha_range=(-0.5, 0.5), beta_range=(-0.5, 0.5), resolution=25
    )
    
    print("🔍 Computing Transformer loss landscape...")
    X_gpt, Y_gpt, Z_gpt = compute_loss_landscape(
    ...
    print("  - loss_landscape_3d_comparison.png")
    print("  - loss_landscape_contours.png")
    
    return {
        'gfn_roughness': gfn_roughness,
        'gpt_roughness': gpt_roughness,
        'smoothness_ratio': gfn_smoothness / gpt_smoothness
    }
