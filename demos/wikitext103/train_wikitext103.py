...
class WikiText103Custom:
    def __init__(self, root='.data', split='train'):
        self.root = Path(root)
        self.split = split
        
    def read_tokens(self):
        print(f"Loading WikiText-103 {self.split} split from HuggingFace...")
        try:
...
def train_wikitext103():
    ...
    # 1. Load Data
    train_loader_raw = WikiText103Custom(split='train')
    val_loader_raw = WikiText103Custom(split='valid')
