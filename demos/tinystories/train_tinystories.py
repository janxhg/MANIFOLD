...
class SimpleVocab:
    def __init__(self, tokens, min_freq=10):  # Increased from 2 to 10
...
class TinyStoriesDataset:
    def __init__(self, split='train', max_stories=500000):
        self.split = split
        self.max_stories = max_stories  # Limit to avoid memory issues
    ...
    def read_tokens(self):
        ...
        # Limit dataset size
        total_stories = min(len(dataset), self.max_stories)
        print(f"Processing {total_stories:,} stories (limited from {len(dataset):,})...")
        ...
        for i in range(0, total_stories, chunk_size):
            ...
            chunk_text = ' <eos> '.join(chunk['text'])
            chunk_tokens = re.findall(r'\S+', chunk_text.lower())
            all_tokens.extend(chunk_tokens)
            
            # Progress update every 100K stories
            if i > 0 and i % 100000 == 0:
                print(f"  Processed {i:,}/{total_stories:,} stories ({len(all_tokens):,} tokens so far)")
        
        print(f"✓ Loaded {len(all_tokens):,} tokens from {total_stories:,} stories")
        return all_tokens
...
def train_tinystories():
    ...
    # 1. Load Data (limit to 500K stories to avoid memory issues)
    train_loader_raw = TinyStoriesDataset(split='train', max_stories=500000)
    val_loader_raw = TinyStoriesDataset(split='validation', max_stories=50000)
    
    train_tokens = train_loader_raw.read_tokens()
    val_tokens = val_loader_raw.read_tokens()
    
    # 2. Build Vocab from Train (min_freq=10 to keep vocab small)
    vocab = SimpleVocab(train_tokens, min_freq=10)
    ...
    for epoch in range(config['training']['epochs']):
        ...
        torch.save({
            ...
            'val_loss': val_loss,
            'vocab': vocab,  # Save vocab for generation
