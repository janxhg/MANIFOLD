...
def train_convergence(model, task, max_steps=5000, lr=1e-3, device='cuda'):
    # Transformer usually needs generous LR but warmup... using AdamW standard
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=200)
    criterion = nn.CrossEntropyLoss()
    model.train()
    
    pbar = tqdm(range(max_steps), desc=f"Transformer Sorting")
    normalized_loss = 5.0 
...
def run():
    ...
    print("Initialize MicroGPT (Transformer Baseline)...")
    model = MicroGPT(
    ...
    # Same steps (1000) and LR (3e-3) as Manifold for fair comparison
